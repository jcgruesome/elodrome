...
- Exactly one revision attempt per verify failure, sharing the existing critique-revision budget rather than adding a second one (single mode) / reusing the existing `revise()` helper (tournament mode) — see Task 6/7 for why this keeps worst-case worker calls unchanged from today.
