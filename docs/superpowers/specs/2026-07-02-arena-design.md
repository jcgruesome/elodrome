...
| Contestants | 3 (or 2 if catalog thin): top-2 Elo for the profile + 1 explorer (fewest matches); all `toolCalling: reliable` |
| Failures | Contestant error = **forfeit** = loss in every pairwise result → degraded endpoints demote themselves (free health-probing) |
...
delegate(task, workspace, task_profile)
   ...
   └─ else ARENA (mode:'tournament')
        ...
        ├─ 2 blind judges rank + list per-entry issues (strict JSON, 1 retry)
        ├─ rank-sum aggregate; tie → fewest completion tokens
        ├─ winner revision round iff judges flagged substantive issues; re-validate
        ├─ Elo update per profile tag (pairwise, forfeits = losses), matches++
        └─ trace kind:'tournament'
```

## Components

1. **Registry schema v2** (`src/registry/schema.ts`)
   Each model gains `ratings: { [tag]: { elo: number; matches: number } }`
   (defaults 1000 / 0). `outcomes` and `evalScore` remain. `version: 2`.
   Loader migrates v1 files in memory (empty ratings) and rewrites as v2 on
   first write; any other shape throws (fail-fast unchanged).

2. **Contestant selector** (`src/arena/select.ts`)
   *Primary tag* = first tag of `task_profile`. *Dominant champion* = top-Elo
   model for the primary tag with `matches ≥ 5` **and** Elo lead ≥ 100 over
   the runner-up → return single-route decision. Otherwise pick contestants:
   top-2 Elo for the profile + 1 explorer = eligible model with fewest
   matches on the primary tag (ties → lowest id, deterministic). Eligibility:
   all profile tags present, `toolCalling: 'reliable'`, not the explorer twice.
   Fewer than 2 eligible → throw (Claude decides; no silent single fallback).
   An explicit `model` param bypasses the arena entirely (v1 behavior).
...
3. **Arena engine** (`src/arena/arena.ts`)
   ...
   contestant that throws (degraded endpoint, budget, malformed submit)
   **forfeits**: recorded with its error message, excluded from judging,
   counted as a loss in every pairwise result. All contestants forfeit →
   throw an error naming every forfeit reason; no Elo change.
...
4. **Judge panel** (`src/arena/judge.ts`)
   ...
   0 → throw. Entries are anonymized: stable shuffle to
   labels A/B/C; judges see task, per-entry summary/rationale/diffs, and
   validation results — never model names. Each judge returns strict JSON
   `{ ranking: string[], issues: { [label]: string[] } }` with the same
   extract-first-JSON-block + one-retry discipline as v1 critique; a judge
   failing both attempts is dropped (1-judge panel noted in response); both
   failing → throw. Aggregate: rank-sum; tie → fewest completion tokens.

5. **Elo updater** (`src/arena/elo.ts`)
   Final ranking (forfeits last) → all pairwise results → standard Elo,
   K=32, applied per tag in `task_profile`; `matches` increments per
   tournament per tag. Pure functions; registry write is a single immutable
   read-modify-write after the tournament completes (crash mid-tournament
   writes nothing). `report_outcome` additionally adjusts the run's worker
...
7. **Surfaces**
   ...
   - CLI: `nva leaderboard [--tag code-gen] [--export md]` — the markdown
     ...
     match counts, sourced from the registry).
...
- All contestants forfeit → throw with every forfeit reason; no Elo change.
- One judge unusable → 1-judge panel, noted in `arena.judges`; both → throw.
- < 2 eligible contestants → throw naming the eligibility gap.
- Registry: v1 migrates, anything else throws. Elo write all-or-nothing.
- No silent fallbacks anywhere; forfeits are data, not hidden retries.

## Testing

- **Unit:** Elo math (pairwise conversion, K-factor, forfeit-as-loss,
  outcome nudges); selector (gate thresholds, explorer determinism,
  eligibility throw, explicit-model bypass); anonymization (model names never
  appear in judge prompts — load-bearing for blindness); rank-sum + token
  tie-break; registry v1→v2 migration.
- **Pipeline (scripted clients):** full tournament happy path; one forfeit;
  all forfeit; judge disagreement; judge failure ×1 and ×2; winner revision
  on substantive judge issues; single-path gate (dominant champion).
...
| Risk | Mitigation |
|---|---|
| Judge bias (verbose diffs win) | Blind labels, 2-judge rank-sum, efficiency tie-break; judge-Elo deferred to v2 |
| Rate-limit pressure on review models | Panel of 2, one ranking call each (not per-entry); per-model queue already exists |
| Elo instability at low match counts | Gate requires ≥5 matches before exploitation; explorer keeps sampling |
| Same-sandbox contention | Sandbox is read-only; no shared mutable state between contestants |
| Free-tier catalog churn | Forfeit mechanism demotes dead endpoints automatically |
